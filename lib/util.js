...
module.exports = {
  getFilenameFromUrl(pubPath, compiler, url) {
    ...
    const hasDifferentHostName =
      localPrefix.hostname !== null &&
      urlObject.hostname !== null &&
      !hostNameIsTheSame;

    // publicPath is not in url, so it should fail
    const publicPathNotInUrl =
      publicPath && hostNameIsTheSame && url.indexOf(publicPath) !== 0;

    // localPrefix.path is not in url, so it should fail
    const localPrefixPathNotInUrl =
      !urlObject.hostname &&
      localPrefix.hostname &&
      url.indexOf(localPrefix.path) !== 0;

    if (hasDifferentHostName || publicPathNotInUrl || localPrefixPathNotInUrl) {
      return false;
    }
