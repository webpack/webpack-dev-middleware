...
module.exports = function Shared(context) {
	var share = {
		setOptions: function(options) {
			...
			if(typeof options.dontServeBundleWithErrors === "undefined") {
				options.dontServeBundleWithErrors = false;
			}
		...
		compilerDone: function(stats) {
			...
			process.nextTick(function() {
				...
				// don't execute callbacks if we were asked to not
				// serve bundle with compilation errors
				if(context.options.dontServeBundleWithErrors && stats.hasErrors()) return;
		...
		ready: function ready(fn, req) {
			...
			if(context.state && !(context.options.dontServeBundleWithErrors && context.webpackStats.hasErrors())) {
