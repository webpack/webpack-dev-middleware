...
// Precompile `src/options.json` into a standalone validator, `src/options.check.js`.
//
// Validating the options compiles the schema with ajv on the first call, which
// cost ~160ms of the first `middleware()` — a one-time startup price paid on
// every dev server start. The generated validator answers the same question
// with no compile step, so the happy path never loads ajv at all; the real
// validator is only reached when the options are invalid, to build the
...
const SCHEMA_PATH = path.join(ROOT, "src", "options.json");
const OUTPUT_PATH = path.join(ROOT, "src", "options.check.js");
