...
| `statsOptions`         | `hot.statsOptions`, but deprecated and removed in the next major: the payload is fixed (`name`, `action`, `time`, `hash`, `errors`, `warnings`), and `hash`, `timings` and `children` cannot be overridden. Drop warnings at the source with webpack's [`ignoreWarnings`](https://webpack.js.org/configuration/other-options/#ignorewarnings), or hide them on the client with `?overlay={"warnings":false}`. |
